...
(function () {
  ...
  //  a byte-for-byte copy of stuart-onboarding.jsx's parser, differing only in
  ...
  function DonOnboarding({ viewerIsManager = false, viewerRepId = "", onFlag } = {}) {
    const model = useMemo(() => window.parseInduction(window.DON_INDUCTION_MD || "", PACK_ROOT), []);
  ...
  window.donInductionSummary = function (items) {
    const model = window.parseInduction(window.DON_INDUCTION_MD || "", PACK_ROOT);
    const m = items || {};
