// ============================================================
//  Stuart — Onboarding  ·  src/stuart-onboarding.jsx
//
//  Renders Stuart Chadwick's EMEA induction as a live, checkable checklist.
//  Content is parsed at runtime from window.STUART_INDUCTION_MD
//  (src/stuart-induction-content.js). This is a Stuart-specialised copy of
//  don-onboarding.jsx — kept as a separate file (matching the don-* per-rep
//  convention) so Don's live view carries zero regression risk. If a third
//  rep ever needs one, factor the shared body out into a makeOnboarding()
//  factory rather than cloning a third time.
//
//  Persistence is PER-USER and durable, via the same Supabase round-trip the
//  rest of the dashboard uses (window.loadInductionState /
//  window.setInductionItem — both key on the logged-in user, so no per-rep
//  wiring is needed), with a localStorage fallback. Checkboxes store '1'/'0';
//  the access-notes textarea stores its text.
// ============================================================
(function () {
  const { useState, useEffect, useRef, useMemo } = React;

  // Day 1 of the induction == Stuart's start (the draft is dated 2026-07-13).
  const TODAY_LABEL = "Monday, July 13, 2026";

  // Resource pack — shared OneDrive folder. ⚠ TBC: this points at the NA
  // enablement pack (the product/competitor collateral is shared with EMEA);
  // swap for the EMEA-specific pack folder + per-file share links once they
  // exist. PACK_ROOT must match the REF prefix used in the content file so
  // parseRef strips it for the displayed relative path.
  ...
  const PACK_ROOT = "EMEA Sales/Enablement/Onboarding/";

  // ----------------------------------------------------------
  //  Parser — markdown → structured model (issue #30 item 3: this used to be
  //  a byte-for-byte copy of don-onboarding.jsx's parser, differing only in
  //  the PACK_ROOT string baked into parseRef's closure. Now shared as
  //  window.parseInduction(md, packRoot) in src/onboarding-parser.js,
  //  parameterized on the one thing that ever differed.)
  // ----------------------------------------------------------
  ...
  // Per-item inline note — used on each access/login row so Stuart can flag a
    ...
    if (readOnly) {
      ...
      return (
        <div className="ind__inote ind__inote--ro">
          <span className="ind__inote-rolabel">Stuart noted:</span>
  ...
  //  Main view
  // ----------------------------------------------------------
  function StuartOnboarding({ viewerIsManager = false, viewerRepId = "", onFlag } = {}) {
    const model = useMemo(() => window.parseInduction(window.STUART_INDUCTION_MD || "", PACK_ROOT), []);
    // Manager (anyone who isn't Stuart) sees Stuart's induction READ-ONLY.
    const readOnly = !!viewerIsManager && viewerRepId !== "stuart";
      ...
      (async () => {
        try {
          const loaded = readOnly
            ? (window.loadInductionStateFor ? await window.loadInductionStateFor("stuart") : {})
    ...
    // Manager summary — everything Stuart has logged about access/logins.
    ...
    return (
      <div className={"ind" + (readOnly ? " is-readonly" : "")} data-screen-label="05 Stuart Onboarding">
        {/* Header */}
        <div className="ind__head">
          <div className="ind__eyebrow"><span className="dot" /> New starter · Induction</div>
          <h1 className="ind__title">Stuart Chadwick — Onboarding</h1>
        ...
        {readOnly && (
          <div className="ind__mgr">
            <div className="ind__mgr-banner">
              <span className="ind__mgr-dot" /> Read-only — this is <strong>Stuart's</strong> induction. You see his live
              progress and notes; only Stuart can check items off.
            </div>
            <div className="ind__mgr-notes">
              <div className="ind__mgr-notes-h">Access notes from Stuart</div>
  ...
  window.StuartOnboarding = StuartOnboarding;
  ...
  window.stuartInductionSummary = function (items) {
    const model = window.parseInduction(window.STUART_INDUCTION_MD || "", PACK_ROOT);
    const m = items || {};
